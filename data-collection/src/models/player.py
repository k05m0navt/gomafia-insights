...
class PlayerData(BaseModel):
    ...
    current_nickname: str = Field(..., min_length=1, description="Current player nickname")
    ...
    @validator('go_mafia_id')
    def extract_id_from_url_if_needed(cls, v, values):
        ...
        if 'profile_url' in values:
            ...
            if match:
                ...
                if isinstance(v, int) and v != extracted_id:
                    logger.warning(f"Player ID mismatch: provided {v}, URL contains {extracted_id}")
                return extracted_id
        return v
    
    @validator('win_rate', 'civilian_win_rate', 'mafia_win_rate', 'don_win_rate', 'sheriff_win_rate')
    def validate_win_rate(cls, v, values):
    ...
    def validate_data(self) -> ValidationResult:
        ...
        if self.games_won > self.games_played:
            result.add_error(f"Games won ({self.games_won}) cannot exceed games played ({self.games_played})")
        
        if self.games_played > 0:
            calculated_win_rate = self.games_won / self.games_played
            if abs(calculated_win_rate - self.win_rate) > 0.01:
                result.add_warning(f"Win rate inconsistency: calculated {calculated_win_rate:.3f}, stored {self.win_rate:.3f}")
        
        # Role games consistency
        total_role_games = self.civilian_games + self.mafia_games + self.don_games + self.sheriff_games
        if total_role_games > self.games_played:
            result.add_error(f"Total role games ({total_role_games}) exceeds total games played ({self.games_played})")
        
        # ELO validation
        if self.max_elo < self.current_elo:
            result.add_warning(f"Max ELO ({self.max_elo}) is less than current ELO ({self.current_elo})")
    ...
    def from_scraped_data(cls, raw_data: dict) -> 'PlayerData':
        ...
        try:
            ...
            for rate_field in ['win_rate', 'civilian_win_rate', 'mafia_win_rate', 'don_win_rate', 'sheriff_win_rate']:
                if rate_field in raw_data and isinstance(raw_data[rate_field], str):
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to parse {rate_field}: {raw_data[rate_field]}")
...
class GameParticipationData(BaseModel):
    ...
    @validator('elo_change', always=True)
    def calculate_elo_change(cls, v, values):
    ...
    @validator('personal_outcome', always=True)
    def determine_personal_outcome(cls, v, values):
        ...
        if 'game_outcome' in values and 'team_side' in values:
            ...
            elif (game_outcome == TeamOutcome.TOWN_WIN and team_side == TeamSide.TOWN) or \
    ...
    @validator('game_duration', always=True)
    def calculate_game_duration(cls, v, values):
    ...
    def validate_data(self) -> ValidationResult:
        ...
        # Required field validation
        required_fields = ['player_go_mafia_id', 'game_identifier', 'seat_position', 
                          'nickname_at_time', 'role', 'team_side', 'elo_before', 'elo_after']
        if not self.validate_required_fields(required_fields):
            result.add_error("Missing required game participation fields")
        
        # Role and team consistency
        if self.role in [PlayerRole.DON, PlayerRole.MAFIA] and self.team_side != TeamSide.MAFIA:
            result.add_error(f"Role {self.role} should be on MAFIA team, not {self.team_side}")
        elif self.role in [PlayerRole.CIVILIAN, PlayerRole.SHERIFF, PlayerRole.DOCTOR] and self.team_side != TeamSide.TOWN:
            result.add_error(f"Role {self.role} should be on TOWN team, not {self.team_side}")
    ...
    def from_scraped_data(cls, raw_data: dict) -> 'GameParticipationData':
        ...
        try:
            ...
            if 'role' in raw_data:
                ...
                elif role in [PlayerRole.CIVILIAN, PlayerRole.SHERIFF, PlayerRole.DOCTOR]:
            ...
            # Parse points
            for points_field in ['base_points', 'bonus_points', 'penalty_points', 'total_points']:
                if points_field in raw_data and isinstance(raw_data[points_field], str):
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to parse {points_field}: {raw_data[points_field]}")
            ...
            logger.error(f"Failed to create GameParticipationData from scraped data: {e}")
            logger.error(f"Raw data: {raw_data}")
            raise 