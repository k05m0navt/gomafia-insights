...
class TournamentData(BaseModel):
    ...
    def validate_current_participants(cls, v, info):
        ...
        if v is not None and "max_participants" in values:
            ...
            if max_p is not None and v > max_p:
                raise ValueError(
                    f"Current participants ({v}) cannot exceed maximum ({max_p})"
                )
        return v

    @field_validator("tournament_type")
    def determine_tournament_type(cls, v, info):
    ...
    def validate_data(self) -> ValidationResult:
        ...
        if self.current_participants and self.max_participants:
            if self.current_participants > self.max_participants:
                result.add_error(
                    "Current participants ({}) exceeds maximum ({})".format(
        ...
        if self.total_rounds and self.current_round > self.total_rounds:
            result.add_error(
                "Current round ({}) exceeds total rounds ({})".format(
...
class GameData(BaseModel):
    ...
    @field_validator("civilian_count", mode="after")
    def calculate_civilian_count(cls, v, info):
    ...
    def validate_data(self) -> ValidationResult:
        ...
        if self.end_time and self.start_time:
            ...
            if (
                calculated_duration < 5 or calculated_duration > 300
            ):  # 5 minutes to 5 hours
                result.add_warning(
                    f"Unusual game duration: {calculated_duration:.1f} minutes"
                )
        ...
        if self.mafia_count + self.civilian_count != self.player_count:
            ...
            if total_roles > 0:  # Only warn if we have role counts
                result.add_warning(
                    f"Player count mismatch: {total_roles} roles "
                    f"vs {self.player_count} players"
                )
        ...
        if self.player_count > 0 and self.mafia_count > 0:
            ...
            if mafia_ratio < 0.2 or mafia_ratio > 0.4:
                result.add_warning(
                    f"Unusual mafia ratio: {mafia_ratio:.2f} "
                    f"({self.mafia_count}/{self.player_count})"
                )
